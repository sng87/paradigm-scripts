#!/usr/bin/env python
"""cytoscapewebReport.py: Generate cytoscapeweb sessions from a bundle (.sif and .NA files)

Usage:
  cytoscapewebReport.py [options] bundleDir reportDir
...
<html>
    ...
    <head>
        ...
        <script type="text/javascript" src="%s/js/min/json2.min.js"></script>
        <script type="text/javascript" src="%s/js/min/AC_OETags.min.js"></script>
        <script type="text/javascript" src="%s/js/min/cytoscapeweb.min.js"></script>
...
htmlTail = """                ';
                ...
                var options = {
                    swfPath: "%(rd)s/swf/CytoscapeWeb",
                    flashInstallerPath: "%(rd)s/swf/playerProductInstall"
                };
...
<html>
    ...
    <head>
        ...
        <script type="text/javascript" src="%s/js/jquery.tablesorter.js"></script>
        <script type="text/javascript">
...
def main(args):
    ## parse arguments
    try:
        opts, args = getopt.getopt(args, "t:r:q")
    ...
    for i in os.listdir(featureDir):
        if i.endswith(".sif"):
            ...
            break
    (nodes, interactions) = rSIF(sifFile)
    ...
    for i, node in enumerate(nodes.keys()):
        idMap[node] = i+1
        nodeMap[i+1] = node
    labelMap = r2Col("%s/LABEL.NA" % (featureDir), delim = " = ", header = True)
    typeMap = r2Col("%s/TYPE.NA" % (featureDir), delim = " = ", header = True)
    scoreMap = r2Col("%s/SCORE.NA" % (featureDir), delim = " = ", header = True)
    nodes = nodeMap.keys()
    nodes.sort()
    ...
    o.write(htmlHead % (resourceDir, resourceDir, resourceDir))
    o.write(graphmlContent)
    o.write(htmlTail % ({"rd" : resourceDir}))
    o.close()
    if os.path.exists("%s/img" % (featureDir)):
    ...
    for file in tableFiles:
        if file.startswith("link:"):
            ...
            if line.startswith(">"):
                ...
                o.write(htmlTableHead % (resourceDir))
